...
def summarize_culture_plates(
    run_dir: Path, pooled: Mapping[str, int] | None = None
) -> list[CulturePlateSummary]:
    """Per compressed plate, how much of the pooled diversity each well recovered."""
    ...
    for row in _read_csv_gz(path):
        ...
        for name in source.split("|"):
            ...
            per_source[row["plate_id"]][name] += 1
    return [
        CulturePlateSummary(
            plate_id=plate_id,
...
def culture_plate_figure(
    summaries: Sequence[CulturePlateSummary], path: Path
) -> Path | None:
    """Deconvolution made visible: which pooled culture plate each well recovered.

    The plate map answers "did this well see everything that was pooled into it",
    which is the question the compression creates.  The bars beside it answer
    "did every pooled plate contribute", which is where a failed transformation
    or a weak culture shows up.
    """
    ...
    for index, summary in enumerate(summaries):
        ax = figure.add_subplot(grid[index, 0])
        observed = list(summary.sources_per_well.values())
...
def write_all(
    ...
    culture = culture_plate_figure(
        summarize_culture_plates(run_dir, pooled),
        output_dir / "fig4_culture_plates",
    )
    if culture is not None:
        written.append(culture)
    return written
